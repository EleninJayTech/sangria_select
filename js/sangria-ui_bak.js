...
let SangriaUI={
	...
	UI_SELECT:{
		...
		setOption:function(selectSelector){
			let _this = this;
			let targetName = _this.makeOptionHtml(selectSelector);
		...
		makeHtml:function(){
			...
			$(`select${_this.targetSelector}`).each(function(){
				...
				_this.makeOptionHtml(`select[data-ss-name="${targetName}"]`);
				_this.setSelectText(targetName);
			});
		},
		 ...
		 * @returns {string}
		 */
		makeOptionHtml:function(selectSelector){
			let el_select = $(selectSelector);
			let targetName = el_select.attr('data-ss-name');
			$(`.ss_option_list[data-ss-name="${targetName}"]`).remove();
			let selectOptionHtml = `<ul class="ss_option_list" data-ss-name="${targetName}">`;
			el_select.find('option').each(function(){
				let el_in_this = $(this);
				let in_this_value = el_in_this.val();
				let in_this_text = el_in_this.text();
				selectOptionHtml += `<li data-ss-name="${targetName}" data-ss-value="${in_this_value}" title="${in_this_text}">${in_this_text}</li>`;
			});
			selectOptionHtml += `</ul>`;
			$(`.ss_wrap[data-ss-name="${targetName}"]`).append(selectOptionHtml);
			return targetName;
		},
